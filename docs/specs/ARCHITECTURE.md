...
- **Thesis-ingested answers (NIL-54) are in-band.** Once ingested they are `player_answers` like any others and pool into these aggregates by design — for thesis pairs the "prior" and the ingested data are the same 36 humans, so pooling live on top is exactly the self-norming intent. No segregation column (see §12).
